...
const ActiveTripScreen: React.FC = () => {
    const { state, updateTrip } = useAppContext();
    ...
    const handleStopTrip = () => {
        ...
        updateTrip({ ...activeTrip, status: 'Completed', endTime: Date.now(), totalTimeSeconds: finalTime });
    };
    ...
    if (!activeTrip) {
        return (
            <div className="bg-white p-6 rounded-lg shadow text-center text-gray-500">
                <p>No active trip.</p>
                <p className="text-sm mt-2">Go to the Trips screen to start a planned trip.</p>
            </div>
        );
    }
...
const StopCard: React.FC<StopCardProps> = ({ stop, stopIndex, tripId }) => {
    ...
    const { elapsedSeconds, isActive, formattedTime, start, pause } = useTimer(stop.timeSpentSeconds);
    ...
    const updateTimeSpent = () => {
        ...
        const newStops = activeTrip.stops.map(s => 
            s.workOrderId === stop.workOrderId ? {...s, timeSpentSeconds: elapsedSeconds} : s
        );
        updateTrip({...activeTrip, stops: newStops});
    }
    ...
    const completeStop = () => {
        ...
        const newStops = activeTrip.stops.map(s => s.workOrderId === stop.workOrderId ? {...s, isCompleted: true, timeSpentSeconds: finalTime} : s);
        updateTrip({...activeTrip, stops: newStops});
        updateWorkOrder({...order, status: OrderStatus.Completed, completedDate: new Date().toISOString()});
    }
